...
const FormField = ({
  ...
  // Custom onChange handler for select fields
  ...
  const field = (
    ...
    >
      {select && [
        <MenuItem key="select-empty" value="">
          -select-
        </MenuItem>,
        ...options.map((opt) => {
          // Handle different data structures for display
          let displayValue =
            opt.name ||
            opt.company_name ||
            opt.agreement_title ||
          ...
          // If still no display value, use id
          if (!displayValue) {
            displayValue = opt.id || "Unknown";
          }
...
const OnJobTrainingIndex = () => {
  ...
  const fetchApi = async (service, params, setter, errorMsg) => {
    try {
      ...
      console.log(`Fetched data from ${service.name}:`, response.data);
  ...
  const fetchDzongkhags = () =>
    fetchApi(
      ...
      [],
      (data) => {
        // Map the data to ensure consistent structure with string IDs
  ...
  const getInitialValues = (type, data = null) => {
    const maps = {
      ojt: {
        firmId: data?.company_id ? String(data.company_id) : "",
      ...
      firm: {
        ...
        dzongkhag: data?.dzongkhag_id ? String(data.dzongkhag_id) : "",
        address: data?.address || "",
        description: data?.description || "",
      },
      placement: {
        ojtAgreementId: data?.agreement_id ? String(data.agreement_id) : "",
        traineeCid: data?.trainee_cid || "",
        traineeName: data?.trainee_name || "",
        courseId: data?.course_id ? String(data.course_id) : "",
  ...
  const FirmForm = ({ formik }) => {
    // Prepare dzongkhag options with proper display values and string IDs
    const dzongkhagOptions = dzongkhags.map((item) => ({
      id: String(item.id || item.dzonkhagId),
      name: item.dzonkhagName || item.name || item.dzonkhag || `ID: ${item.id}`,
    }));
