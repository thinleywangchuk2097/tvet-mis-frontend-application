...
const ViewInstituteMonitoringIndex = () => {
  ...
  const fetchInstituteMonitoringAssessment = async () => {
    ...
    try {
      const response =
        await MonitoringAssessmentService.getMonitoringAssessmentByApplicationNo(
          applicationNo,
          access_token,
        );
      const data = Array.isArray(response.data)
        ? response.data
        : [response.data];
      setInstituteMonitoringDetails(data);
  ...
  const handleSubmit = async (statusId, actionType) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        toast.success(
          <div>
            <strong>Checklist {actionType}d Successfully!</strong>
            <br />
    ...
    <Paper sx={{ p: 2, mt: 1 }}>
      <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
        ...
        <Typography variant="h6">
          Institute Monitoring Checklist - Application No: {applicationNo}
        </Typography>
      </Box>
        ...
        <Box>
          <Box sx={{ mb: 2, p: 2, bgcolor: "#f5f5f5", borderRadius: 1 }}>
            <Typography variant="subtitle2" color="text.secondary" gutterBottom>
              <strong>Institute:</strong> {selectedInstitute?.institute_name} |{" "}
              <strong>Registration No:</strong>{" "}
              {selectedInstitute?.registration_no} | <strong>Dzongkhag:</strong>{" "}
              {getDzongkhagName(selectedInstitute?.dzongkhag_id)} |{" "}
              <strong>Monitoring Date:</strong>{" "}
              {selectedInstitute?.monitoring_date}
            </Typography>
            <Typography variant="subtitle2" color="text.secondary">
              <strong>Current Status:</strong>{" "}
              {selectedInstitute?.status || "Pending Review"}
            </Typography>
          </Box>
