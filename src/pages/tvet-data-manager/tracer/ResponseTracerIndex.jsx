...
const ResponseTracerIndex = () => {
  ...
  // Column definitions for display (without Response Rate)
  const columns = [
  ...
  const handleDownloadSelected = () => {
    ...
    // Convert to CSV with all columns
  ...
  const handleDownloadAll = () => {
    ...
    const csvData = filteredResponses.map((row) => {
      return columns
        .map((col) => {
          const value = row[col.id];
          // Handle values with commas
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `all_responses_${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
    window.URL.revokeObjectURL(url);
  };
                        ...
                        <TableCell align="center">
                          ...
                          >
                            ...
                            >
                              <span>
                                {" "}
                                {/* Span needed for tooltip to work with disabled button */}
                                <IconButton
                                  size="small"
                                  sx={{
                                    bgcolor: "#e8f5e8",
